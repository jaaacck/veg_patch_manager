...
class JobSerializer(serializers.ModelSerializer):
    veg = serializers.PrimaryKeyRelatedField(
        queryset=VegEntry.objects.all(), required=False, allow_null=True)
    plant = serializers.PrimaryKeyRelatedField(
        queryset=Plant.objects.all(), required=False, allow_null=True)

    class Meta:
        model = Job
        fields = ['id', 'veg', 'plant', 'month', 'description', 'created_at']


class SeedlingSerializer(serializers.ModelSerializer):
    veg = serializers.PrimaryKeyRelatedField(queryset=VegEntry.objects.all())
    veg_key = serializers.CharField(source='veg.key', read_only=True)
    veg_display = serializers.CharField(source='veg.display_name', read_only=True)

    class Meta:
        model = Seedling
        fields = ['id', 'veg', 'veg_key', 'veg_display', 'date_sown',
                  'amount', 'sprouted', 'failed', 'notes',
...
class PlantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'water_level', 'sun_level', 'soil_type', 'jobs',
            'created_at', 'updated_at',
        ]
...
class VegEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = VegEntry
        fields = [
            'key', 'name', 'variety', 'display_name', 'latin_name', 'emoji', 'image', 'image_url',
            'jobs',
...
class PhotoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Photo
        fields = ['id', 'cell', 'image', 'image_url', 'caption', 'taken_on', 'created_at']
